...
BASE_MAGMOM = "3.8000 -3.8000 3.8000 -3.8000 3.8000 -3.8000 -3.8000 3.8000 4*4.4"
BASE_ATOMS = 12
...
for d in subdirs:
    ...
    # 기존 INCAR에서 MAGMOM이 적힌 줄은 모두 필터링하여 제거
    new_incar_lines = [line for line in incar_lines if not line.strip().upper().startswith('MAGMOM')]
    ...
    print(f"[{d}] 업데이트 완료 (총 원자: {total_atoms}개 -> 추가된 0: {diff}개)")
